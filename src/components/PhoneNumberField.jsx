...
function looksLikeBareNumber(val) {
  const digits = val.replace(/\D/g, "");
  // 10 digits with no leading + strongly suggests a North American number missing +1
  return digits.length === 10 && !val.trim().startsWith("+");
}
...
export default function PhoneNumberField() {
  ...
  const showCountryCodeHint = looksLikeBareNumber(phone);
    ...
    <div style={{ marginTop: "1.5rem" }}>
      ...
      <form onSubmit={handleSave} style={{ display: "flex", gap: "0.6rem", alignItems: "center" }}>
        <input
          ...
          style={{
            background: "var(--bg)",
            border: `1px solid ${showCountryCodeHint ? "goldenrod" : "var(--border)"}`,
            borderRadius: "0.6rem",
      ...
      {showCountryCodeHint ? (
        <div style={{ fontSize: "0.7rem", color: "goldenrod", marginTop: "0.4rem", lineHeight: 1.5 }}>
          Did you include your country code? e.g. +1 for Canada/US
        </div>
      ) : (
        <div style={{ fontSize: "0.7rem", opacity: 0.35, marginTop: "0.4rem", lineHeight: 1.5 }}>
          {saved
            ...
            : `Optional. Add your number to receive a text before each stillness moment. Include country code e.g. +1 for Canada/US.`
          }
        </div>
      )}
