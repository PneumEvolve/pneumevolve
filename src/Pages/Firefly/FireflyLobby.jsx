// src/Pages/Firefly/FireflyLobby.jsx
import React, { useState } from "react";
import { api } from "@/lib/api";
import { Link } from "react-router-dom";

export default function FireflyLobby({ onRoomReady }) {
  const [joinCode, setJoinCode]   = useState("");
  const [loading, setLoading]     = useState(false);
  const [error, setError]         = useState(null);
  const [waiting, setWaiting]     = useState(false); // P1 created, waiting for P2

  async function handleCreate() {
    setLoading(true);
    setError(null);
    try {
      const { data } = await api.post("/firefly/rooms");
      setWaiting(true);
      onRoomReady(data, "p1");
    } catch (e) {
  ...
  async function handleJoin() {
    ...
    setError(null);
    try {
      const { data } = await api.post("/firefly/rooms/join", { join_code: code });
      onRoomReady(data, "p2");
    } catch (e) {
  ...
  return (
    <main className="min-h-screen bg-[#080c14] text-white flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm space-y-8">

        {/* Title */}
        <div className="text-center space-y-2">
          <h1 className="text-3xl font-light tracking-widest" style={{ color: "rgba(200,255,150,0.9)" }}>
            firefly
          </h1>
        ...
        <div className="rounded-xl border p-4 space-y-2 text-xs leading-relaxed"
          ...
          <p>
            <span style={{ color: "rgba(200,255,150,0.7)" }}>Player 1</span> moves through darkness.
            Fireflies are your only light — collect as many as you can in 3 minutes.
          </p>
          <p>
            <span style={{ color: "rgba(255,255,255,0.6)" }}>Player 2</span> sees the whole map
            and guides Player 1 around walls and the zombie.
          </p>
        </div>
        ...
        <div className="space-y-3">
          <button
            ...
            style={{
              ...
              border: "1px solid rgba(200,255,150,0.25)",
        ...
        <Link to="/" className="block text-center text-xs"
